...
in
{
  strip.test-strips-from-functionArgs-attr = {
    expr = let
      mod = { __functionArgs = { host = false; config = true; lib = true; }; __functor = _: _: { }; };
      stripped = stripBindingArgs { module = mod; bindingNames = [ "host" ]; };
    in stripped.__functionArgs;
    expected = { config = true; lib = true; };
  };
  ...
  strip.test-strips-from-raw-function = {
    ...
    expr = let
      ...
      stripped = stripBindingArgs { module = mod; bindingNames = [ "host" ]; };
    in stripped.__functionArgs;
    expected = { config = false; lib = false; };
  };
  ...
  strip.test-noop-when-nothing-to-strip = {
    ...
    expr = let
      mod = { config, lib, ... }: { };
      stripped = stripBindingArgs { module = mod; bindingNames = [ "host" ]; };
    in builtins.functionArgs stripped;
    expected = { config = false; lib = false; };
  };

  strip.test-noop-for-attrset = {
    expr = let
      mod = { services.nginx.enable = true; };
      stripped = stripBindingArgs { module = mod; bindingNames = [ "host" ]; };
    in mod == stripped;
    expected = true;
  };
