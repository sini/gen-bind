...
| `buildSignature` | `{ module, bindings, defaultMergeStrategy, mergeStrategies, provenance ? {}, vocabulary ? null } -> { requires; bound; unsatisfied; mergeStrategies; }` |
...
| ★ `crossing.mkSystemTerminal` | `{ evaluator, locateConfig } -> { adapter = carriage -> Adapter; locateConfig; }`. `bindFormals` is `wrapAll`'s partial application over a module-list `Body` and returns `.all` — the merge-collision validators cross with the modules, so a collision is NAMED in the target's `warnings` channel under the retired surface's own message family (warn-and-proceed; the only throw is the consumer's own per-value `_mergeStrategy = "error"` opt-in). The price: a validator defines `warnings`, so the target must declare that option — the retired surface's own imposition. `extent`/`extraModules`/`passthrough`/`evaluator` ride the closure, invisible to δ and to `E(u)`. ★ CARRIAGE AND TARGET ARE TWO CONTRACTS, DELIBERATELY UNWELDED: the carriage names are the delivery surface's, the target-facing keys are `nodes` plus whatever the consumer put in `passthrough`, and neither derives from the other. ★ The channel may NOT carry `nodes`: `//` gives it priority, so it would silently replace the peer set — refused by name, on the channel's SPINE only, so the opacity holds |
