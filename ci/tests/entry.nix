...
in
{
  ...
  flake.tests.entry.test-standalone-entry-matches-lib = {
    ...
    expected = builtins.attrNames genBind;
  };
  ...
  flake.tests.entry.test-control-the-compared-surface-is-non-trivial = {
    expr = builtins.length (builtins.attrNames standalone) > 12;
    expected = true;
  };
  ...
  flake.tests.entry.test-control-the-comparison-discriminates = {
    expr = builtins.attrNames standalone == builtins.attrNames (removeAttrs genBind [ "wrap" ]);
    expected = false;
  };
  ...
  # builds a thunk and recognises it again, so the constructor and its predicate both run.
  flake.tests.entry.test-the-shims-library-is-live = {
    expr = standalone.isThunk (standalone.mkThunk (_: 1));
    expected = true;
  };
